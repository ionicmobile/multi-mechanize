...
def output_results(results_dir, results_file, run_time, rampup, ts_interval, user_group_configs=None, xml_reports=False):
    ...
    if user_group_configs:
        ...
        report.write_line('<tr><th>group name</th><th>threads</th><th>script name</th><th>description</th></tr>')
        for user_group_config in user_group_configs:
            report.write_line('<tr><td>%s</td><td>%d</td><td>%s</td><td>%s</td></tr>' %
                (user_group_config.name, user_group_config.num_threads, user_group_config.script_file, user_group_config.description))
        report.write_line('</table>')
